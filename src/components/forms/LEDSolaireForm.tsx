...
import { ledSolaireStep1Schema, ledSolaireStep2ProSchema, type LEDSolaireStep1Data, type LEDSolaireStep2ProData } from "@/lib/validations/led-solaire";
...
export const LEDSolaireForm = () => {
  ...
  const form1 = useForm<LEDSolaireStep1Data>({
    ...
    defaultValues: {
      ...
      zone_type: "",
      sun_exposure: "",
      lamppost_height: undefined,
      postal_code: "",
    },
  });

  const form2 = useForm<LEDSolaireStep2ProData>({
    resolver: zodResolver(ledSolaireStep2ProSchema),
    defaultValues: {
      company_name: "",
      siren: "",
      employees: "",
  ...
  const onStep1Submit = (data: LEDSolaireStep1Data) => {
    setStep1Data(data);
    setStep(2);
  };

  const onStep2Submit = async (data: LEDSolaireStep2ProData) => {
    if (!step1Data) return;
        ...
        <Form {...form1}>
          <form onSubmit={form1.handleSubmit(onStep1Submit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Nombre de points lumineux extérieurs</FormLabel>
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Exposition au soleil</FormLabel>
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue placeholder="Sélectionnez" />
            ...
            <FormField
              control={form1.control}
              name="lamppost_height"
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Hauteur des lampadaires actuels (m)</FormLabel>
                  <FormControl>
                    <Input type="number" step="0.1" placeholder="ex: 6" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}

...
export const particulierMultiSchema = z.object({
  selected_aids: z.array(z.enum([
    'isolation', 'pac', 'panneaux_solaires',
    'brasseur_air', 'housse_piscine', 'aides_energetiques'
  ...
  // Champs conditionnels - Piscine
  piscine_volume: z.number().optional(),
  piscine_heating: z.string().optional(),
  piscine_target_temp: z.number().optional(),
  ...
  if (data.selected_aids.includes('housse_piscine')) {
    return data.piscine_volume && data.piscine_heating;
  }
  return true;
}, {
  message: "Les détails de la piscine sont requis",
  path: ["piscine_volume"]
});

export type ParticulierMultiFormData = z.infer<typeof particulierMultiSchema>;
